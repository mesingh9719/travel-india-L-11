<x-admin.master-layout pageTitle="Owners" pageSubTitle="Edit Owners">
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center w-100 mt-2">
                        <div>
                            <h4 class="card-title mb-2 mt-2">Edit Owners</h4>
                        </div>
                    </div>
                </div>
				                    ...
				                    <div class="form-group">
				                        <label for="{{ $column }}">{{ ucwords(str_replace('_', ' ', $column)) }}</label>
				                            @if($column == "rc_image_front")
				                                ...
				                                @else
				                                 <input type="text" name="{{ $column }}" id="{{ $column }}" class="form-control" value="{{ old($column) }}" placeholder="Enter {{ ucwords(str_replace('_', ' ', $column)) }}">
				                                @endif
				                        @error($column)
